...
import {PgUtil, sql} from './types/pg.slonik';
...
const rwPgUtilProvider: FactoryProvider = {
    ...
    useFactory: async (configService: ConfigService<Config>) => {
        ...
        const pool = new PgUtil(rwPgConfig, PG_UTIL_DEFAULT_TIMEOUT, READ_WRITE);
        return pool;
    },
};
...
const roPgUtilProvider: FactoryProvider = {
    ...
    useFactory: async (configService: ConfigService<Config>) => {
        ...
        const pool = new PgUtil(roPgConfig, PG_UTIL_DEFAULT_TIMEOUT, READ_ONLY);
        return pool;
    },
};
